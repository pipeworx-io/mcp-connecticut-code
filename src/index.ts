 ...
 *
 * Connecticut publishes by CHAPTER, and the chapter is nowhere in the
 * citation: "Conn. Gen. Stat. § 53a-54a" lives in chapter 952. Its title index
 * does publish the ranges, though —
 *
 *   Chapter 950  Secs. 53a-1 to 53a-3
 *   Chapter 951  Secs. 53a-4 to 53a-23
 *   Chapter 952  Secs. 53a-24 to 53a-323
 *
 * so the chapter is resolved from the state's own index rather than from a
 * table that would rot. The chapter file is then large (chapter 952 is about a
 * megabyte of text covering the whole penal code), so the requested section is
 * cut out of it — returning the chapter would answer one question with three
 * hundred statutes.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Connecticut General Statutes');
}

const BASE = 'https://www.cga.ct.gov/current/pub';
const UA = 'pipeworx-mcp-connecticut-code/1.0 (+https://pipeworx.io)';

/** "53a-54a" -> 54 ; letter suffixes do not affect ordering here. */
function secNum(s: string): number {
  const m = /-(\d+)/.exec(s);
  return m ? parseInt(m[1], 10) : NaN;
}

async function chapterFor(title: string, section: string): Promise<string | null> {
  const res = await pwFetch(`${BASE}/title_${encodeURIComponent(title)}.htm`, {
    headers: { 'User-Agent': UA },
  });
  if (!res.ok) return null;
  const text = statuteText(await readStatuteResponse(res));
  const want = secNum(section);
  if (!Number.isFinite(want)) return null;
  const re = /Chapter\s+(\d+)\s+Secs?\.\s*([0-9a-zA-Z-]+)\s*(?:to|-|—)\s*([0-9a-zA-Z-]+)/g;
  for (let m = re.exec(text); m; m = re.exec(text)) {
    const lo = secNum(m[2]), hi = secNum(m[3]);
    if (Number.isFinite(lo) && Number.isFinite(hi) && want >= lo && want <= hi) return m[1];
  }
  return null;
}

const tools: McpToolExport['tools'] = [{
  name: 'ct_general_statute',
  description:
  ...
  inputSchema: {
    type: 'object',
    properties: {
      section: { type: 'string', description: 'Section as cited, e.g. "53a-54a" (murder) or "14-227a" (DUI). The part before the dash is the title.' },
    },
    required: ['section'],
  },
}];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'ct_general_statute') throw new Error(`Unknown tool: ${name}`);
  ...
  if (!m) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not a Connecticut citation.`,
      hint: 'Connecticut sections are title-section, e.g. "53a-54a" (murder) or "14-227a" (DUI).' };
  }
  const title = m[1];

  const chapter = await chapterFor(title, raw);
  if (!chapter) {
    return { found: false, reason: 'section_not_found', section: raw, title,
      message: `No chapter in Connecticut title ${title} covers section ${raw}.`,
      hint: 'The chapter is resolved from Connecticut’s own title index; a section outside every published range does not exist in that title.' };
  }

  const url = `${BASE}/chap_${chapter}.htm`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (!res.ok) throw new Error(`Connecticut General Assembly returned HTTP ${res.status} for chapter ${chapter}`);
  const text = statuteText(await readStatuteResponse(res));

  const esc = raw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const body = sliceSection(
    text,
    new RegExp(`Sec\\.\\s*${esc}\\.`, 'g'),
    new RegExp(`Sec\\.\\s*\\d+[a-zA-Z]?-\\d+[a-zA-Z]*\\.`, 'g'),
  );
  if (!body) {
  ...
  const heading = (body.match(/^Sec\.\s*[\da-zA-Z-]+\.\s*([^.]{2,140})\./) ?? [])[1] ?? null;

  return {
    citation: `Conn. Gen. Stat. § ${raw}`, section: raw, title, chapter,
    heading: heading ? heading.trim() : null,
    text: body, text_chars: body.length, url,
    source: 'Connecticut General Assembly (cga.ct.gov), keyless',
    attribution: 'Connecticut statutes are public record.',
  };
}
